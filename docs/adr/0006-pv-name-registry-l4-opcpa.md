...
**Status:** Partially superseded by [ADR-0010](0010-per-laser-yaml-config.md) and [ADR-0012](0012-in-repo-config.md) — read/write *signal* PV names now live as full strings in the module's per-zone YAML, not assembled by builders. Only the **command** vocabulary (`LASER_COMMANDS` + `pv.cmd`) remains in `pv-names.ts`.
**Date:** 2025-09-15
