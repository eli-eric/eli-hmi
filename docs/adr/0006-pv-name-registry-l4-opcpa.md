...
**Status:** Partially superseded by [ADR-0009](0009-per-laser-yaml-config.md) — read/write *signal* PV names now live as full strings in `config/lasers.yaml`, not assembled by builders. Only the **command** vocabulary (`LASER_COMMANDS` + `pv.cmd`) remains in `pv-names.ts`.
**Date:** 2025-09-15
