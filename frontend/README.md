...
CONFIG_DIR=/app/zone-config                 # zone-config dir; unset in dev → ../eli-hmi-config
...
Per-environment config — navigation, allowed routes, and per-module PV config — lives **outside the app build** in a zone-config directory ([ADR-0011](../docs/adr/0011-runtime-zone-config.md)):
...
- The middleware (`src/middleware.ts`, Node runtime) gates routes via `src/lib/settings/zone-service.ts`; the nav bar (client) receives `navigationItems`/`homeRoute` from `/api/runtime-config` via `useRuntimeConfig()`.
