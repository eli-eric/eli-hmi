...
One directory = everything a deployment needs: which pages exist, what shows
in the top navigation, and every PV name each module displays.

**This folder in the app repo is a ready-to-copy template.** The intended
long-term home is a **separate git repo owned by the controls team** — copy
this whole directory as its initial commit (see [Copying out](#copying-out-as-a-standalone-repo)).
Until then it doubles as the local-development default config.
...
```
┌ config repo (this layout) ┐        ┌ frontend container ┐
│ zones/L4.yaml             │ volume │ CONFIG_DIR=/app/…  │
│ modules/l4-opcpa/…        │ ─────► │ ZONE_CODE=L4       │
...
- The deployment **mounts this directory** into the frontend container
  ...
  - `ZONE_CODE` — which zone this deployment is (e.g. `L4`, `TESTZ`)
...
## Copying out as a standalone repo

1. Copy this whole directory as the initial commit of the new repo.
2. Add CI validation — see [`ci-example.yml`](ci-example.yml). The job checks
   out the app repo at a pinned tag and runs `validate:config` against this
   repo, so broken config is caught at commit time, not at deploy.
